...
- **`--rule-strong` is a control boundary, not a hairline.** It is sized to clear 3:1 against its surface in both schemes so fields and quiet buttons stay identifiable (WCAG 1.4.11); `--rule` is the decorative hairline. Softening `--rule-strong` for looks re-breaks every input and secondary button at once.
- **The theme has three states, not two.** An explicit choice stamps `data-theme` on the root element and is stored locally; the default stamps nothing and leaves `prefers-color-scheme` to decide. Every scheme-dependent rule therefore needs all three selectors — bare `:root`, the media query guarded by `:root:not([data-theme="light"])`, and `:root[data-theme="dark"]` — or a stored choice silently loses to the system preference. `components/theme-toggle.tsx` also exports the head script that applies the stored value before first paint; which label shows is decided in CSS rather than React state, because the server cannot know the reader's scheme.
